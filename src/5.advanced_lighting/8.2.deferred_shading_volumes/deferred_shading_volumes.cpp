...
int main()
{
    ...
    // -------------------------
    utils::Shader shaderGeometryPass("8.2.g_buffer.vs", "8.2.g_buffer.fs");
    utils::Shader shaderLightingPass("8.2.deferred_shading.vs", "8.2.deferred_shading.fs");
    utils::Shader shaderLightBox("8.2.deferred_light_box.vs", "8.2.deferred_light_box.fs");
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, gPosition,
                           0);
    // normal color buffer
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, gNormal,
                           0);
    ...
    for (unsigned int i = 0; i < NR_LIGHTS; i++)
    {
        ...
        float rColor =
            static_cast<float>(((rand() % 100) / 200.0f) + 0.5); // between 0.5 and 1.)
        float gColor =
            static_cast<float>(((rand() % 100) / 200.0f) + 0.5); // between 0.5 and 1.)
        float bColor =
            static_cast<float>(((rand() % 100) / 200.0f) + 0.5); // between 0.5 and 1.)
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (unsigned int i = 0; i < lightPositions.size(); i++)
        {
            ...
            const float constant = 1.0f; // note that we don't send this to the shader, we
                                         // assume it is always 1.0 (in our case)
        ...
        // 2.5. copy content of geometry's depth buffer to default framebuffer's depth
        // buffer
        // ----------------------------------------------------------------------------------
        ...
        // formats are implementation defined. This works on all of my systems, but if it
        // doesn't on yours you'll likely have to write to the depth buffer in another
