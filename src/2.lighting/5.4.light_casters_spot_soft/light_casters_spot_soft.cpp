...
int main()
{
    ...
    // ------------------------------------
    utils::Shader lightingShader("5.4.light_casters.vs", "5.4.light_casters.fs");
    utils::Shader lightCubeShader("5.4.light_cube.vs", "5.4.light_cube.fs");
    ...
    glBindBuffer(GL_ARRAY_BUFFER, VBO);
    // note that we update the lamp's position attribute's stride to reflect the updated
    // buffer data
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void *)0);
    glEnableVertexAttribArray(0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // conditions differ with each lighting method and environment. each environment
        // and lighting type requires some tweaking to get the best out of your
        // environment.
        ...
        // again, a lamp object is weird when we only have a spot light, don't render the
        // light object lightCubeShader.use(); lightCubeShader.setMat4("projection",
        // projection); lightCubeShader.setMat4("view", view); model = glm::mat4(1.0f);
