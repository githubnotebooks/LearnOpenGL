...
int main()
{
    ...
    // -------------------------
    utils::Shader shader("2.stencil_testing.vs", "2.stencil_testing.fs");
    utils::Shader shaderSingleColor("2.stencil_testing.vs", "2.stencil_single_color.fs");
    ...
    float cubeVertices[] = {
        ...
        0.5f,  0.5f,  0.5f,  1.0f, 0.0f, 0.5f,  0.5f,  0.5f,  1.0f, 0.0f,
        -0.5f, 0.5f,  0.5f,  0.0f, 0.0f, -0.5f, 0.5f,  -0.5f, 0.0f, 1.0f};
    float planeVertices[] = {// positions          // texture Coords (note we set these
                             // higher than 1 (together with GL_REPEAT as texture wrapping
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // draw floor as normal, but don't write the floor to the stencil buffer, we only
        // care about the containers. We set its mask to 0x00 to not write to the stencil
        // buffer.
        glStencilMask(0x00);
        // floor
        glBindVertexArray(planeVAO);
        ...
        // several 1s. The parts of the buffer that are 1 are not drawn, thus only drawing
