...
int main()
{
    ...
    // -------------------------
    utils::Shader pbrShader("2.2.1.pbr.vs", "2.2.1.pbr.fs");
    utils::Shader equirectangularToCubemapShader("2.2.1.cubemap.vs",
                                          "2.2.1.equirectangular_to_cubemap.fs");
    utils::Shader irradianceShader("2.2.1.cubemap.vs", "2.2.1.irradiance_convolution.fs");
    utils::Shader prefilterShader("2.2.1.cubemap.vs", "2.2.1.prefilter.fs");
    utils::Shader brdfShader("2.2.1.brdf.vs", "2.2.1.brdf.fs");
    utils::Shader backgroundShader("2.2.1.background.vs", "2.2.1.background.fs");
    ...
    // pbr: set up projection and view matrices for capturing data onto the 6 cubemap face
    // directions
    // ----------------------------------------------------------------------------------------------
    ...
    // then let OpenGL generate mipmaps from first mip face (combatting visible dots
    // artifact)
    glBindTexture(GL_TEXTURE_CUBE_MAP, envCubemap);
    ...
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // generate mipmaps for the cubemap so OpenGL automatically allocates the required
    // memory.
    glGenerateMipmap(GL_TEXTURE_CUBE_MAP);

    // pbr: run a quasi monte-carlo simulation on the environment lighting to create a
    // prefilter (cube)map.
    // ----------------------------------------------------------------------------------------------------
    prefilterShader.use();
    ...
    // then re-configure capture framebuffer object and render screen-space quad with BRDF
    // shader.
    glBindFramebuffer(GL_FRAMEBUFFER, captureFBO);
    glBindRenderbuffer(GL_RENDERBUFFER, captureRBO);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, 512, 512);
    ...
    // then before rendering, configure the viewport to the original framebuffer's screen
    // dimensions
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // this looks a bit off as we use the same shader, but it'll make their positions
        // obvious and keeps the codeprint small.
        for (unsigned int i = 0; i < sizeof(lightPositions) / sizeof(lightPositions[0]);
             ++i)
        {
            glm::vec3 newPos =
