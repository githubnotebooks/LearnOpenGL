...
int main()
{
    ...
    // vertex attribute configuration remains the same (still one 3-float position vector
    // per vertex)
    float vertices[] = {
        // first triangle
        -0.9f, -0.5f, 0.0f, // left
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindVertexArray(
            VAO); // seeing as we only have a single VAO there's no need to bind it every
